    ...
    <main class="admin-main">
      ...
      <header class="admin-topbar">
        <div class="topbar-left">
          <h2 class="topbar-title">Events</h2>
          <div class="topbar-breadcrumb">
            <span>Admin</span><span class="bc-sep">›</span>
            <span class="bc-active">Events</span>
          </div>
        </div>
      ...
      <div class="content-wrap" :class="{ visible: mounted }">
        ...
        <div class="page-header">
          <div>
            ...
            <p class="page-sub">{{ events.total }} events total</p>
          </div>
        ...
        <!-- Table -->
        <div class="panel">
          <table class="data-table" v-if="events.data.length">
            <thead>
              <tr>
                <th>Title</th>
                <th>Type</th>
            ...
            <tbody>
              <tr v-for="ev in events.data" :key="ev.id">
                <td class="td-title">
                  ...
                  <div class="post-slug">{{ ev.slug }}</div>
                </td>
                <td>
                  <span class="tag-type">{{ ev.event_type }}</span>
                </td>
                <td class="td-date">{{ formatDate(ev.event_date) }}</td>
                <td>
                  <span class="status-badge" :class="`status-${ev.status}`">{{ ev.status }}</span>
                </td>
                <td class="td-center">
                  <span v-if="ev.is_featured" class="featured-star" title="Featured">★</span>
                  <span v-else class="td-na">—</span>
                </td>
                <td class="td-actions">
                  <Link :href="`/admin/events/${ev.id}/edit`" class="btn-action btn-edit" title="Edit">
                  ...
                  <button @click="confirmDelete(ev)" class="btn-action btn-delete">
          ...
          <div v-else class="empty-state">
            <svg viewBox="0 0 40 40" fill="none" width="40" height="40"><rect x="6" y="8" width="28" height="24" rx="2" stroke="currentColor" stroke-width="1.2"/><path d="M13 4v8M27 4v8M6 16h28" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>
            <p>No events found.</p>
            <Link href="/admin/events/create" class="btn-primary-sm">Create your first event</Link>
          </div>

          <!-- Pagination -->
          <div v-if="events.last_page > 1" class="pagination">
            <Link
    ...
    <div v-if="deleteTarget" class="modal-overlay" @click.self="deleteTarget = null">
      <div class="modal">
        ...
        <div class="modal-actions">
          ...
          <button class="btn-danger" @click="deleteEvent">Delete</button>
        </div>
      </div>
    </div>
...
.tag-type{display:inline-block;padding:3px 9px;background:rgba(201,168,76,.1);color:#9a7a2e;font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;border:1px solid rgba(201,168,76,.25);}
.td-na{color:#c0c8d4;}
